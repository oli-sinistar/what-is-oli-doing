...
**Schema v3 gives the AI schedule creative + managerial control**: `theme.hue` re-tints the entire page daily (aurora, glows, heatmap ramp — all derived via `oklch` from one hue), `flair.line` is its daily one-liner, `activity.calendar` (91 days) feeds a GitHub-style heatmap with streak detection and week-over-week delta, and `goals` (≤5, sourced from Linear **and** commitments stated in Claude/Cursor chats) each carry an `eta` — the goals card renders a management week-outlook: "N closing by Friday", ⚑ chips on this-week commitments, ✓ on shipped. Hand-rolled visual layer: canvas particle constellation, cursor-spotlight card borders, rotating conic hero ring, terminal-style deep-dive with typewriter, radial goal rings, count-up stats, marquee ticker, film grain — all reduced-motion-safe.
